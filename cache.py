...
class CacheLine:
    def __init__(self, block_size):  # Fixed method name from _init_ to __init__
...
class CacheSet:
    def __init__(self, associativity, block_size):  # Fixed method name from _init_ to __init__
        self.associativity = associativity
        self.cache_set = [CacheLine(block_size) for i in range(associativity)]


class Cache:
    def __init__(self, cache_size, block_size, associativity, replacement_policy):  # Fixed method name from _init_ to __init__
        # replacement_policy: 0 for LRU, 1 for MRU
        self.cache_size = cache_size
        self.block_size = block_size
        self.associativity = associativity
        self.replacement_policy = replacement_policy  # 0 for LRU, 1 for MRU/RRPV
        ...
        self.max_rrpv = 3
        self.mru = [[0]*self.associativity for _ in range(self.number_of_sets)]
        self.init_lru()
        self.init_mru()
    ...
    def init_mru(self):
        for i in range(self.number_of_sets):
            for j in range(self.associativity):    
                self.mru[i][j] = self.max_rrpv  # Initialize all to max_rrpv
        ...
        for i in range(len(self.c[index].cache_set)):
            ...
            if cache_line.tag == tag:
                ...
                if self.replacement_policy == 0:  # LRU
                    ...
                    self.update_mru_cache_hit(index, i)
    ...
    def update_mru_cache_hit(self, index, cache_line_index):
        self.mru[index][cache_line_index] = 0

    def update_mru(self, index, cache_line_index):
        # Mark this line as recently used (RRPV = max_rrpv - 1)
        self.mru[index][cache_line_index] = self.max_rrpv - 1
    ...
    def get_replace_line_mru(self, index):
        ...
        for i in range(self.associativity):
            if self.mru[index][i] == self.max_rrpv:
                return i
        ...
        # After incrementing, there should be at least one line with max_rrpv
        return self.get_replace_line_mru(index)
    ...
    def replace_line(self, address, cache_line_data, any_value_changed):
        ...
        if self.replacement_policy == 0:  # LRU
            ...
            self.update_mru(index, line_to_replace_index)
